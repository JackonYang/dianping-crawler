...
for f in os.listdir('D:\github\data\shop_review'):#搜索目标文件夹中的文件名（店铺及评价页面），输出在revp.json中
    revp_ret = revp_ptn.findall(f)                     #key为店铺id，对应的values为爬取到的店铺评价页
    for k,v in revp_ret:
        if k in revp_list:
